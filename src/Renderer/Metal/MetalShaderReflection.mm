...
struct BufferStructMember {
	...
	BufferStructMember()
			: sizeInBytes(0), offset(0), bufferIndex(0) {
		name[0] = '\0';
	}
};
...
struct BufferInfo {
	...
	BufferInfo()
			: sizeInBytes(0), alignment(0), bufferIndex(-1), currentOffset(0), isUAV(false), isArgBuffer(false) {
	}
};
...
void addShaderResource(ShaderResource *pResources,
											 uint32_t idx,
											 DescriptorType type,
		...
		for (uint32_t i = 0; i < pReflectionInfo->buffers.size(); ++i) {
			...
			if (!isInputVertexBuffer(bufferInfo, shaderStage)) {
				if (bufferInfo.isArgBuffer) {
					...
					addShaderResource(pResources,
														resourceIdx,
														DESCRIPTOR_TYPE_ARGUMENT_BUFFER,
						...
						if (bufferMember.bufferIndex == bufferInfo.bufferIndex) {
							...
							switch (bufferMember.descriptor.mDataType) {
								case MTLDataTypeTexture:
									...
									break;
								case MTLDataTypeSampler:descriptorType = DESCRIPTOR_TYPE_SAMPLER;
									break;
								case MTLDataTypeArray:
								case MTLDataTypePointer:descriptorType = DESCRIPTOR_TYPE_BUFFER;
									break;
								default: ASSERT(0);
